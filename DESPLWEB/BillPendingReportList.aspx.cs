...
namespace DESPLWEB
{
    public partial class BillPendingReportList : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //lblheading.Text = "List of reports pending for bill generation";
                lblheading.Text = "List of bills not generated because of reports not yet ready";
                optViewList.Checked = true;
                optGenerateBill.Enabled = false;
                var user = dc.User_View(Convert.ToInt32(Session["LoginId"].ToString()), 0, "", "", "");
                foreach (var u in user)
                {
                    if (u.USER_Approve_right_bit == true)
                    {
                        optGenerateBill.Enabled = true;
                    }
                }
            }
        }
        ...
        {
            if (optViewList.Checked == true)
            {
                var pendingCl = dc.Bill_View_ReportPendingList(false);
                grdReport.DataSource = pendingCl;
                grdReport.DataBind();
                grdReport.Columns[7].Visible = false;
            }
            else if (optGenerateBill.Checked == true)
            {
        ...
        protected void optGenerateBill_CheckedChanged(object sender, EventArgs e)
        {
            grdReport.DataSource = null;
            grdReport.DataBind();
        }

        protected void optViewList_CheckedChanged(object sender, EventArgs e)
        {
            grdReport.DataSource = null;
            grdReport.DataBind();
        }
