...
namespace DESPLWEB
{
    public partial class ClientHomeMetro : System.Web.UI.Page
    {
        Int32 cnt = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                //else if (arrMsgs[1].ToString().Equals("Metro"))
                ...
                lblMetro.Text = myData.getClientName(Convert.ToDouble(Session["ClientID"].ToString()), ddl_db.SelectedItem.Value);
...
//                LoadSiteList();
        ...
        protected void LoadClientList()
        {
            ...
            dt = myData.getClientforMetro(Convert.ToDouble(Session["ClientID"].ToString()), ddl_db.SelectedItem.Value.ToString());
            ddlClient.DataSource = dt;
            ddlClient.DataTextField = dt.Columns[0].ToString();
            ddlClient.DataValueField = dt.Columns[1].ToString();
            ddlClient.DataBind();
        ...
        protected void LoadSiteList()
        {
            ...
            dt = myData.getSiteListforMetro(Convert.ToDouble(ddlClient.SelectedItem.Value.ToString()), ddl_db.SelectedItem.Value.ToString());
        ...
        protected void LoadListOfSupplier()
        {
            ...
            //dt = myData.getSupplierList(Convert.ToDouble(ddlClient.SelectedItem.Value.ToString()), Convert.ToDouble(ddlSite.SelectedItem.Value.ToString()), ddl_db.SelectedItem.Value);
        ...
        protected void LoadListOfBuilding()
        {
            ...
            dt = myData.getBuildingListMetro(Convert.ToDouble(ddlClient.SelectedItem.Value.ToString()), Convert.ToDouble(ddlSite.SelectedItem.Value.ToString()), ddl_db.SelectedItem.Value);
        ...
        protected void btnDisplay_Click(object sender, EventArgs e)
        {
            ...
            {
                //   ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('Please select Month ');", true);
                lblMonth.Visible = true;
            }
            else if (ddlYear.SelectedIndex <= 0)
            {
                // ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('Please select year ');", true);
                lblMonth.Visible = true;
            }
            else
            {
        ...
        protected void ddllMonth_SelectedIndexChanged(object sender, EventArgs e)
        {
            // LoadListOfBuilding();
            lblMonth.Visible = false;
            grdView.DataSource = null;
        ...
        protected void ViewReport(string RefNo, string RecType)
        {
            ...
            {
                if (ddl_db.SelectedValue == "veena2016")
                {
        ...
        public void bindData()
        {
            ...
            try
            {
                ...
                if ((ddlSite.SelectedIndex > 0) && (ddlMaterial.SelectedIndex > 0) && (ddlMonth.SelectedIndex > 0) && (ddlYear.SelectedIndex>0))
                {
                    ...
                    if (HttpContext.Current.Session["databasename"].ToString() == "veena2016")
                    {
                        dt = myData.getReportList2016Metro(Convert.ToDouble(ddlClient.SelectedItem.Value.ToString()), 
                            Convert.ToDouble(ddlSite.SelectedItem.Value.ToString()),
                            ddlBuildings.SelectedItem.Text, 
                            ddlMaterial.SelectedItem.Value.ToString(), 
                            ddl_db.SelectedItem.Value, "", "---All---",Convert.ToInt32(ddlMonth.SelectedItem.Value)  ,Convert.ToInt32(ddlYear.SelectedItem.Value) );
                    }
                    //else
                    //{
        ...
        {
            PrintPDFReport objrpt = new PrintPDFReport();
            myDataComm obj = new myDataComm();
            string billId;
            if (ddlClient.SelectedIndex > 0 && ddlSite.SelectedIndex > 0 && ddlMonth.SelectedIndex > 0 && ddlYear.SelectedIndex > 0)
            {
                billId = obj.getBill(Convert.ToDouble(ddlClient.SelectedItem.Value), Convert.ToDouble(ddlSite.SelectedItem.Value), Convert.ToInt32(ddlMonth.SelectedItem.Value), Convert.ToInt32(ddlYear.SelectedItem.Value), lblConnectionLive.Text);
                objrpt.Bill_PDFPrint(billId, false, "Print");
            }
        }
        ...
        protected void ddl_db_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //}
            lblConnection.Text = "Data Source=92.204.136.64;Initial Catalog=PuneMetro;User ID=dipl;Password=dipl2020"; 
            LoadSiteList();
        }
